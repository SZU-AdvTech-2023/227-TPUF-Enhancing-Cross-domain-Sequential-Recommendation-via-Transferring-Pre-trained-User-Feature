...
parser.add_argument('--s_model', default='sas', type=str)  # mlp,mf,sas
...
if __name__ == '__main__':
    ...
    [user_train, user_valid, user_test, user_neg, usernum, itemnum, timenum] = dataset
    ...
    try:
        relation_matrix = pickle.load(open('data/relation_matrix/relation_matrix_%s_%d_%d.pickle'%(args.dataset, args.maxlen, args.time_span),'rb'))
    except:
        relation_matrix = Relation(user_train, usernum, args.maxlen, args.time_span)
        pickle.dump(relation_matrix, open('data/relation_matrix/relation_matrix_%s_%d_%d.pickle'%(args.dataset, args.maxlen, args.time_span),'wb'))
    ...
    if args.model == 'TPUF' or args.model == 'TPUF_wo_TFM':
        f = open(os.path.join(args.dataset + '_' + args.model + '_AdamW' + '/results',
                              ...
                              + str(args.combine_weight) + '_' + str(args.hidden_units) + str(args.time_span) + '.txt'), 'w')
    ...
    if args.dataset == 'Movies_and_TV' and (args.model == 'TPUF' or args.model == 'TPUF_wo_TFM'):
        if args.s_model == 'sas':
            data_dir = "s_model/SAS_Books_0.24183365358221245.pickle"
            with open(data_dir, "rb") as fid:
                s_emb = pickle.load(fid)
            s_emb = np.array(s_emb).astype(np.float32)
    elif args.dataset == 'CDs_and_Vinyl' and (args.model == 'TPUF' or args.model == 'TPUF_wo_TFM'):
        if args.s_model == 'sas':
            data_dir = "s_model/SAS_Books_0.24183365358221245.pickle"
            with open(data_dir, "rb") as fid:
                s_emb = pickle.load(fid)
            s_emb = np.array(s_emb).astype(np.float32)
    elif args.dataset == 'Books' and (args.model == 'TPUF' or args.model == 'TPUF_wo_TFM'):
        if args.s_model == 'sas':
            data_dir = "s_model/SAS_Movies_and_TV_0.31237990667032667.pickle"
            with open(data_dir, "rb") as fid:
                s_emb = pickle.load(fid)
            s_emb = np.array(s_emb).astype(np.float32)
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step in range(num_batch):  # tqdm(range(num_batch), total=num_batch, ncols=70, leave=False, unit='b'):
            ...
            if args.model == 'SASRec':
                ...
                pos_logits, neg_logits, pos_embs, neg_embs = model(u, seq, time_matrix, pos, neg, s_embs)
    ...
    if args.model == 'SASRec':
        ...
        folder = args.dataset + '_' + args.model + '_AdamW'
        fname = args.model + '.dataset={}.epoch={}.lr={}.hidden={}.maxlen={}.l2={}.loss_w{}.combine_w{}_HR_{:.4f}.{}.pth'
        fname = fname.format(args.dataset, epoch, args.lr, args.hidden_units, args.maxlen, args.l2_emb, args.loss_weight, args.combine_weight, t_test[2], now_time)
        torch.save(model.state_dict(), os.path.join(folder, fname))
        print(fname)         
